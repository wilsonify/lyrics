...
def genius_get_artists(artist_name):
  ...
  try:
    ...
    return max(result.iteritems(), key=lambda x: x[1])[0]  
  except:
...
def genius_get_songs(artist_id):
  ...
  try:
    ...
    print "no songs found. usage: genius_get_songs(id) return list of tuples [(title,url)...]"
    raise
...
def genius_get_lyrics(song_url):
  ...
  try:
    ...
    print "cannot find lyrics. usage: genius_get_lyrics(song_url) returns lyrics as unicode string"
    raise
...
def genius_get_all_lyrics_for_artist(artist):
  ...
  try:
    ...
    for song in song_list:
      lyricFile = open('/home/thom/projects/Lyrics/'+art[1]+'/'+song[0].replace('/','_')+'.txt','wb')
      surl=song[1]
    ...
    print "cannot get lyrics usage: genius_get_all_lyrics_for_artist(artist) creates one text file per song retrieved from genius.com"
    raise
