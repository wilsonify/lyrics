...
# trains a sequence to sequence (seq2seq) model for translation.
# For example *"¿todavia estan en casa?"*, and return the English translation: *"are you still at home?"*
...
logging.info("Download the file")
...
def create_dataset(path, _num_examples):
    """
    ...
    word_pairs = [
        [preprocess_sentence(w) for w in l.split("\t")] for l in lines[:_num_examples]
    ]
...
def load_dataset(path, _num_examples=None):
    """
    # creating cleaned input, output pairs
    """
    _targ_lang, _inp_lang = create_dataset(path, _num_examples)

    _input_tensor, inp_lang_tokenizer = tokenize(_inp_lang)
    _target_tensor, targ_lang_tokenizer = tokenize(_targ_lang)

    return _input_tensor, _target_tensor, inp_lang_tokenizer, targ_lang_tokenizer
...
class Encoder(tf.keras.Model):
    def __init__(self, vocab_size, _embedding_dim, enc_units, batch_sz):
        super(Encoder, self).__init__()
        self.batch_sz = batch_sz
        self.enc_units = enc_units
        self.embedding = tf.keras.layers.Embedding(vocab_size, _embedding_dim)
...
class BahdanauAttention(tf.keras.Model):
    def __init__(self, _units):
        ...
        self.W1 = tf.keras.layers.Dense(_units)
        self.W2 = tf.keras.layers.Dense(_units)
    ...
    def call(self, query, values):
        ...
        context_vector = _attention_weights * values
        context_vector = tf.reduce_sum(context_vector, axis=1)

        return context_vector, _attention_weights


class Decoder(tf.keras.Model):
    def __init__(self, vocab_size, _embedding_dim, dec_units, batch_sz):
    ...
    def call(self, x, hidden, enc_output):
        ...
        context_vector, _attention_weights = self.attention(hidden, enc_output)
        ...
        return x, state, _attention_weights
...
def train_step(_inp, _targ, _enc_hidden):
    loss = 0

    with tf.GradientTape() as tape:
        enc_output, _enc_hidden = encoder(_inp, _enc_hidden)

        dec_hidden = _enc_hidden

        dec_input = tf.expand_dims([targ_lang.word_index["<start>"]] * BATCH_SIZE, 1)

        # Teacher forcing - feeding the target as the next input
        for t in range(1, _targ.shape[1]):
            # passing enc_output to the decoder
            predictions, dec_hidden, _ = decoder(dec_input, dec_hidden, enc_output)

            loss += loss_function(_targ[:, t], predictions)

            # using teacher forcing
            dec_input = tf.expand_dims(_targ[:, t], 1)

    _batch_loss = loss / int(_targ.shape[1])
...
def evaluate(sentence):
    ...
    enc_out, _enc_hidden = encoder(inputs, hidden)

    dec_hidden = _enc_hidden
    dec_input = tf.expand_dims([targ_lang.word_index["<start>"]], 0)

    for t in range(max_length_targ):
        predictions, dec_hidden, _attention_weights = decoder(
            dec_input, dec_hidden, enc_out
        )

        # storing the attention weights to plot later on
        _attention_weights = tf.reshape(_attention_weights, (-1,))
        attention_plot[t] = _attention_weights.numpy()
...
if __name__ == "__main__":
    ...
    logging.debug("example_input_batch.shape = {}".format(example_input_batch.shape))
    logging.debug("example_target_batch.shape = {}".format(example_target_batch.shape))
    ...
    [Neural Machine Translation (seq2seq) tutorial](https://github.com/tensorflow/nmt).
     This example uses a more recent set of APIs.
      ...
      The below picture and formulas are an example of attention mechanism 
      from [Luong's paper](https://arxiv.org/abs/1508.04025v5). 
    # 
    # <img src="https://www.tensorflow.org/images/seq2seq/attention_mechanism.jpg" width="500" alt="attention mechanism">
    # 
    # The input is put through an encoder model which gives us the encoder 
    output of shape *(batch_size, max_length, hidden_size)* 
    ...
    #
    """
    [Download a different dataset](http://www.manythings.org/anki/) to experiment with translations,
     for example, English to German, or English to French.
    """
